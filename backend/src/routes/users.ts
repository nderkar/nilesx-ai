...
export async function userRoutes(app: FastifyInstance) {
  ...
  app.get("/users", { preHandler: requireRole(...MANAGE_ROLES) }, async (_request, reply) => {
  ...
  app.get("/users/:id", { preHandler: requireRole(...MANAGE_ROLES) }, async (request, reply) => {
  ...
  app.patch(
    ...
    async (request, reply) => {
      const { id } = request.params as { id: string };
      const body = z.object({ roleName: z.string().min(1) }).parse(request.body);
